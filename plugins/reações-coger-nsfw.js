...
const handler = async (m, { conn, participants, isAdmins }) => {
  ...
  try {
    ...
    const mediaFolder = './media/gifs/coger'
    if (!fs.existsSync(mediaFolder)) {
      fs.mkdirSync(mediaFolder, { recursive: true })
      return conn.sendMessage(from, {
        text: '❌ No hay archivos disponibles. Agrega GIFs o MP4s en media-gifs-coger',
      }, { quoted: m })
    }
    ...
    if (randomFile.toLowerCase().endsWith('.gif')) {
      mp4Path = path.join('./downloads', `coger-${Date.now()}.mp4`)
    ...
    const message = targetUser
      ? `@${senderName}\` *esta cogiendo con* @${targetUser.split('@')[0]}`
      : `*@${senderName}* está disfrutando un buen momento solito`
    ...
    console.log('[COGER] Archivo enviado correctamente.')
  } catch (e) {
    console.error('[COGER] Error:', e)
