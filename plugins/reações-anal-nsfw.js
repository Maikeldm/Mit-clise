...
const handler = async (m, { conn, participants, isAdmins }) => {
  ...
  try {
    ...
    const mediaFolder = './media/gifs/anal'
    if (!fs.existsSync(mediaFolder)) {
      fs.mkdirSync(mediaFolder, { recursive: true })
      return conn.sendMessage(from, {
        text: '❌ No hay archivos disponibles. Agrega GIFs o MP4s en media/gifs/anal',
      }, { quoted: m })
    }
    ...
    if (randomFile.toLowerCase().endsWith('.gif')) {
      mp4Path = path.join('./downloads', `anal-${Date.now()}.mp4`)
    ...
    const message = targetUser
      ? `*@${senderName}* le dio por atrás a *@${targetUser.split('@')[0]}*`
      : `*@${senderName}* está disfrutando un anal solito`
    ...
    console.log('[ANAL] Archivo enviado correctamente.')
  } catch (e) {
    console.error('[ANAL] Error:', e)
