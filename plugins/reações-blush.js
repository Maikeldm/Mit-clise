...
const handler = async (m, { conn, isAdmins }) => {
  ...
  try {
    ...
    let mediaUrl = ''
    try {
      const res = await axios.get('https://nekos.best/api/v2/blush', { timeout: 5000 })
      mediaUrl = res.data?.results?.[0]?.url
      if (!mediaUrl) throw new Error('No se encontró un gif de sonrojo.')
    } catch (e) {
      console.log('[BLUSH] Falló nekos.best:', e.message)
      return conn.sendMessage(from, { text: `❌ Error al obtener gif: ${e.message}` }, { quoted: m })
    }

    const senderName = m.sender.split('@')[0]
    const message = targetUser
      ? `😳 *@${senderName}* sonrojó a *@${targetUser.split('@')[0]}* 🙈`
      : `😳 *@${senderName}* se sonrojó a sí mismo...`

    if (!fs.existsSync('./downloads')) fs.mkdirSync('./downloads')
    const gifPath = `./downloads/blush-${Date.now()}.gif`
    ...
    console.log('[BLUSH] Sonrojo enviado correctamente.')
  } catch (e) {
    console.error('[BLUSH] Error:', e.message)
    conn.sendMessage(from, { text: `❌ Error: ${e.message}` }, { quoted: m })
  }
}

handler.command = ['blush']
